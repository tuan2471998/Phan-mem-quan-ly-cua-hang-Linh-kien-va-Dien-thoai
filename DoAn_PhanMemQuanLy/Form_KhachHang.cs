...
namespace DoAn_PhanMemQuanLy
{
    public partial class Form_KhachHang : Form
    {
        ...
        private void bt_Them_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    int demvip = 0;
                    int demthuong = 0;
                    for (int i = 0; i < count - 1; i++) // đếm số khách hàng vip trong data (count - 1  vì có thêm 1 hàng đc tạo sẵn)
                    {
                        string vip = dataGridView_Form_KhachHang.Rows[i].Cells[1].Value.ToString();
                        if (vip.Substring(0, 4) == mavip)
                            demvip++;
                    }
                    for (int i = 0; i < count - 1; i++) // đếm số khách hàng thường trong data (count - 1  vì có thêm 1 hàng đc tạo sẵn)
                    {
                        string vip = dataGridView_Form_KhachHang.Rows[i].Cells[1].Value.ToString();
                    ...
                    if (cbb_LoaiKhachHang.Text == "VIP")
                        them[0] = mavip + "0" + (demvip + 1).ToString(); // Cấp mã khách hàng vip
                    else
                        them[0] = mathuong + "0" + (demthuong + 1).ToString(); // Cấp mã khách hàng Thường
                    them[1] = txt_HoTen.Text;
                    them[2] = txt_DiaChi.Text;
                    them[3] = txt_DienThoai.Text;
                    if (cbb_LoaiKhachHang.Text == "VIP")
                        them[4] = txt_DienThoai.Text.Substring(7, 3) + "VI0" + (demvip + 1).ToString(); //Cấp số tài khoản cho khách hàng vip
                    else
                        them[4] = txt_DienThoai.Text.Substring(7, 3) + "TH0" + (demthuong + 1).ToString();  //Cấp số tài khoản cho khách hàng Thường
        ...
        private void bt_Xoa_Click(object sender, EventArgs e)
        {
            ...
            catch { MessageBox.Show("Xóa dữ liệu thất bại (-_-)"); }
        }
        ...
        private void bt_CapNhat_Click(object sender, EventArgs e)
        {
            ...
            catch { MessageBox.Show("Cập nhật dữ liệu thất bại (-_-)"); }
        }
        ...
        private void dataGridView_Form_KhachHang_SelectionChanged(object sender, EventArgs e)
        {
            ...
            catch { MessageBox.Show("Chọn dữ liệu thất bại (-_-)"); }
        }
        ...
        private void bt_TimKiem_Click(object sender, EventArgs e)
        {
            try
            {
                if (bt_TimKiem.Text == "Tìm Kiếm")
                {
                    ...
                    string Timkiem = "SELECT * FROM KHACHHANG WHERE TenKH = @TenKH";
                    SqlCommand comd = new SqlCommand(Timkiem, conn);
                    comd.Parameters.AddWithValue("MaKH", txt_MaKH.Text);
                    comd.Parameters.AddWithValue("TenKH", txt_TimKiem.Text);
                    comd.Parameters.AddWithValue("DiaChi", txt_DiaChi.Text);
                    comd.Parameters.AddWithValue("DienThoai", txt_DienThoai.Text);
                    comd.Parameters.AddWithValue("SoTaiKhoan", txt_SoTaiKhoan.Text);
                    comd.Parameters.AddWithValue("NgayDangKy", dtp_KhachHang.Text);
                     comd.Parameters.AddWithValue("LoaiKH", cbb_LoaiKhachHang.Text);
                    comd.ExecuteNonQuery();
                    SqlDataReader re = comd.ExecuteReader();
                    DataTable tb = new DataTable();
                    tb.Load(re);
                    dataGridView_Form_KhachHang.DataSource = tb;
            ...
            catch { MessageBox.Show("Tìm dữ liệu thất bại (-_-)"); }
        }
